...
def main(config_path: str,
    ...
    elif model.workspace is not None:
        pattern = f".*seed_{seed}.*\.ckpt"
        filtered_filenames = [filename for filename in list(os.listdir(workspace)) if re.match(pattern, filename)]
        if len(filtered_filenames) > 0:
            model.load_checkpoint(os.path.join(model.workspace, filtered_filenames[0]), device=device)
